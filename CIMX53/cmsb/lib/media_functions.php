...
function media_getAllMediaRecords() {

  //
  $uploads = mysql_select('uploads', " `tableName` = '_media' AND `recordNum` > 0");
...
function media_replaceUploadsWithMedia($uploads) {
  ...
  foreach ($uploads as $upload) {
    if (empty($upload['mediaNum'])) { continue; }
...
function media_showMediaList() {
  $records = media_getAllMediaRecords();
