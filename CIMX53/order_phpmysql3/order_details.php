...
<html>
  ...
  <tbody>
    ...
    $sql = "SELECT pizza_order.id AS 'order_id',pizza_order.size AS 'size',pizza_order.toppings AS 'toppings',pizza_order.order_total AS 'total',pizza_order.comments,users.first_name, users.last_name,users.address,users.address_2,users.city,users.state,users.zip,users.phone_number FROM `pizza_order` INNER JOIN users ON pizza_order.user_id = users.id WHERE pizza_order.id = ".$_GET['order_id'];
...
<hr>
<h4>Order id: <?php echo $extracted['order_id'];?> </h4>
...
<h3>Total: <?php echo $extracted['total'];?> </h3>
